...
class QuestionList {
    ...
    fun getRandomQuestion(questionType: QuestionType): Question {
        if (questionType == QuestionType.ALL) {
            ...
            return sqlQuestions.random()
        } else if (questionType == QuestionType.JAVA) {
            return javaQuestions.random()
        } else {
            return sqlQuestions.random()
        }
    }
...
//        Question(
//            id = 20,
//            description = "SQL에서 테이블에서 특정 레코드를 선택하려면 어떤 키워드를 사용하나요?",
//            choices = listOf("SELECT", "GET", "FETCH", "PICK"),
//            correctAnswer = 0
//        ),
//        Question(
//            id = 21,
