...
class EmailIntentSenderFactory : ReportSenderFactory {
  override fun create(context: Context, config: CoreConfiguration): ReportSender {
    return EmailIntentSender(config)
  }
}
...
data class MailSenderConfiguration(
  ...
  val reportFileName: String = "report.txt"
...
fun warn(supplier: () -> String) {
  Log.w("EmailIntentSender", supplier())
}
/**
 * Send reports through an email intent.
 *
 * The user will be asked to chose his preferred email client if no default is set. Included report
 * fields can be defined using [org.acra.annotation.AcraCore.reportContent]. Crash receiving mailbox
 * has to be defined with [org.acra.annotation.AcraMailSender.mailTo].
 */
...
class EmailIntentSender(private val config: CoreConfiguration) : ReportSender {
    ...
    val reportText: String =
        try {
          config.reportFormat.toFormattedString(
              errorContent, config.reportContent, "\n", "\n\t", false)
        } catch (e: Exception) {
  ...
  private fun sendLegacy(subject: String, body: String, attachments: List<Uri>, context: Context) {
    ...
    if (resolveActivity != null) {
      if (attachments.isEmpty()) {
        ...
        val altAttachmentIntent =
            Intent(attachmentIntent).apply { type = "*/*" } // gmail will only match with type set
   ...
   * @return email intent
   */
  protected fun buildAttachmentIntent(
      subject: String,
      body: String?,
      attachments: List<Uri>
  ): Intent {
   ...
   * @return email intent
   */
  protected fun buildSingleAttachmentIntent(
      subject: String,
      body: String?,
      attachment: Uri
  ): Intent {
  ...
  protected fun buildFallbackIntent(subject: String, body: String): Intent {
    ...
    intent.data =
        Uri.parse(
            "mailto:${mailConfig.mailTo}?subject=${Uri.encode(subject)}&body=${Uri.encode(body)}")
    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
    intent.putExtra(Intent.EXTRA_SUBJECT, subject)
    intent.putExtra(Intent.EXTRA_TEXT, body)
    return intent
   ...
   */
  protected fun getBodyAndAttachments(
      context: Context,
      reportText: String
    ...
    attachments.addAll(
        InstanceCreator.create(config.attachmentUriProvider) { DefaultAttachmentProvider() }
