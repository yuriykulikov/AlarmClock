...
package com.better.alarm.model.compat;
 ...
 *
 * <p>Use {@link #enqueueWork(Context, Class, int, Intent, long)} to enqueue new work to be
...
public abstract class JobIntentService extends Service {
    ...
    abstract static class WorkEnqueuer {
        ...
        abstract void enqueueWork(Intent work, long deadline);
    ...
    static final class CompatWorkEnqueuer extends WorkEnqueuer {
        ...
        @Override
        void enqueueWork(Intent work, long deadline) {
    ...
    static final class JobWorkEnqueuer extends JobIntentService.WorkEnqueuer {
        ...
        void enqueueWork(Intent work, long deadline) {
            ...
            JobInfo jobInfo = new JobInfo.Builder(jobId, mComponentName)
                    .setOverrideDeadline(deadline)
                    .build();

            mJobScheduler.enqueue(jobInfo, new JobWorkItem(work));
        }
    }
 ...
 * enqueued for the same class.
    ...
    public static void enqueueWork(@NonNull Context context, @NonNull Class cls, int jobId,
                                   @NonNull Intent work, long deadline) {
        enqueueWork(context, new ComponentName(context, cls), jobId, work, deadline);
    }

    /**
     * Like {@link #enqueueWork(Context, Class, int, Intent, long)}, but supplies a ComponentName
 ...
 * enqueued for the same class.
    ...
    public static void enqueueWork(@NonNull Context context, @NonNull ComponentName component,
                                   int jobId, @NonNull Intent work, long deadline) {
        if (work == null) {
